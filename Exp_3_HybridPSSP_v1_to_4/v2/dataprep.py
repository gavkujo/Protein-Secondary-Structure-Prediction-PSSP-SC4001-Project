...
def _read_csv_guess(path: str) -> pd.DataFrame:
    # Some archives use tabs; we attempt CSV first, then gracefully fall back to TSV.
...
def clean_df(df: pd.DataFrame, min_len: int = 20, max_len: int = 2000, drop_nonstd: bool = True) -> pd.DataFrame:
    ...
    if drop_nonstd:
        # Removing non-standard residues keeps the training distribution consistent.
...
class ProteinDataset(Dataset):
    ...
    def __init__(self, csv_path: str, features_dir: Optional[str] = None, max_len: Optional[int] = None):
        # Read the CSV once; the DataLoader workers reuse this in-memory frame.
    ...
    def __getitem__(self, idx):
        ...
        if self.features_dir is not None:
            # Feature files are exported under {pdb_id}_{chain}.npy names; load them lazily.
...
def collate_fn(batch):
    # Sort by decreasing length so pack_padded_sequence can assume an ordered batch later.
    ...
    if available_feats:
        # Allocate a dense feature tensor up front; missing entries remain zero-padded.
...
def prepare_and_split(archive_dir: str, out_dir: str = 'data/processed', min_len: int = 20, max_len: int = 2000,
    ...
    # Convenience wrapper used by the notebooks to regenerate splits with a single command.
